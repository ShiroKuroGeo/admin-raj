          ...
          <div v-else class="row q-col-gutter-md">
            <div v-for="product in getProductsByCategory(category.category_id)" :key="product.product_id" class="col-xs-6 col-sm-4 col-md-3">
              <q-card class="product-card">
                <q-img :src="`http://localhost/admin-raj/database/uploads/${product.image_path}`" :ratio="1" style="height: 200px;" />
                <q-card-section>
...
export default {
  ...
  methods: {
    ...
    async fetchCategories() {
      try {
        const response = await fetch('http://localhost/admin-raj/database/controller/pos_categories.php');
        if (!response.ok) {
    ...
    async fetchProducts() {
      try {
        const response = await fetch('http://localhost/admin-raj/database/controller/pos_products.php');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
